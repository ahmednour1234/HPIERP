<div class="width-inone">
    ...
    <center class="mt-3">
        <h5>{{ \App\CPU\translate('order_ID') }} : {{ $order['id'] }}</h5>
                <h5>{{ \App\CPU\translate('Basic Order') }} : {{ $order['parent_id'??''] }}</h5>
        ...
        <tbody>
            ...
            @foreach ($order->details as $key => $detail)
                @if ($detail->product_details)
                    ...
                    @php($sub_total = number_format($order->order_amount, 2))
    ...
    <dl class="row text-black-50">
        ...
        <dd class="col-5 text-right">{{ number_format($amount, 2) }}</dd>
        <dt class="col-7">{{ \App\CPU\translate('Tax_/_VAT') }}:</dt>
        <dd class="col-5  text-right">{{ number_format($order['total_tax'], 2) }}</dd>
        <dt class="col-7">{{ \App\CPU\translate('subtotal') }}:</dt>
        <dd class="col-5  text-right">{{ $sub_total }}</dd>
  ...
  <div class="mr-1">
    ...
    @if($order->cash == 2)
        {{ \App\CPU\translate('أجل') }}  {{-- For installment --}}
    @else
        {{ \App\CPU\translate('كاش') }}  {{-- For cash --}}
    @endif
</div>
