    ...
    <div class="content container-fluid">
        ...
        <div class="">
            <div class="row align-items-center mb-3">
                <div class="col-sm">
                    <h1 class="page-header-title text-capitalize">{{\App\CPU\translate('pos')}} {{\App\CPU\translate('المرتجعات')}}
                        <span
                            class="badge badge-soft-dark ml-2">{{$refunds->total()}}</span></h1>
                </div>
            </div>
        </div>
        <!-- End Page Header -->
                ...
                >
                 ...
                 <th class="table-column-pl-0">{{\App\CPU\translate('مبيعات')}}</th>
                    ...
                    @foreach($refunds as $key=>$refund)
                        <tr class="status-{{$refund['order_status']}} class-all">
                            <td class="">
                                {{$key+$refunds->firstItem()}}
                            </td>
                            <td class="table-column-pl-0">
                                <a class="text-primary" href="#" onclick="print_invoice('{{$refund->id}}')">{{$refund['id']}}</a>
                            </td>
                            <td>{{ optional($refund->seller)->f_name }} {{ optional($refund->seller)->l_name }}</td>
                         <td>{{ optional($refund->customer)->name }}</td>
                                                  <td>{{ optional($refund->customer->regions)->name }}</td>
                            <td>{{date('d M Y',strtotime($refund['created_at']))}}</td>
<td>{{ $refund['type'] == 4 ? 'مبيع' : 'مرتجع' }}</td>
                            <td>{{ $refund['cash'] == 2 ? 'أجل' : 'كاش' }}</td>
                               <td>
                                {{ ($refund->payment_id != 0) ? ($refund->account ? $refund->account->account : \App\CPU\translate('account_deleted')): 'Customer balance' }}
                            </td>
                            
                            <td>
                                {{ number_format($refund->order_amount, 2) }}
                            </td>
                                                        <td>{{ $refund->extra_discount ? number_format($refund->extra_discount, 2) : 0 }}</td>
                            <td>{{ number_format($refund['total_tax'], 2) }}</td>
<td>
    @if($refund->cash == 1)
...
<td class="none">    <img src="{{ asset('storage/app/public/'.$refund['img']) }}" alt="Image Description" style="width: 50px; height: auto;">
                            ...
                            <td class="none">
                                <button class="btn btn-sm btn-white" target="_blank" type="button"
                                        onclick="print_invoice('{{$refund->id}}')"><i
