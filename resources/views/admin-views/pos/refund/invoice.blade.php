<div class="width-inone">
    ...
    <center class="mt-3">
        ...
        <h5>{{ \App\CPU\translate('seller_name') }} : {{ $order->seller->f_name . ' ' . $order->seller->l_name }}</h5>
        <h5>{{ \App\CPU\translate('customer_name') }} : {{ $order->customer->name }}</h5>
    ...
    <div class="d-flex flex-wrap justify-content-between border-top pt-3">
        ...
        @if ($order->payment_id == 1)
            <div class="mr-1">{{ \App\CPU\translate('amount') }}:
                {{ $order->collected_cash ? number_format($order->collected_cash, 2) : 0 }}
            </div>
