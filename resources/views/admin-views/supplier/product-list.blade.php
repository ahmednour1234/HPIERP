        ...
        <div class="row align-items-center mt-3 mb-3">
            ...
            <div class="col-md-4">
                <a href="{{route('admin.product.add')}}" class="btn btn-primary float-right">
                    <i class="tio-add-circle-outlined"></i>  {{\App\CPU\translate('المنتج')}}
                </a>
            </div>
        </div>
                    ...
                    <div class="row justify-content-between align-items-center flex-grow-1">
                        ...
                        <div class="col-md-4 mt-2">
                            <select name="qty_ordr_sort" class="form-control" onchange="location.href='{{ url('/') }}/admin/supplier/products/{{ $supplier->id }}?sort_oqrderQty='+this.value">
                                ...
                                <option value="quantity_asc" {{ $sort_oqrderQty== "quantity_asc"?'selected':''}}>{{\App\CPU\translate('quantity_sort_by_(بالكمية من الاقل للاعلي)')}}</option>
                                <option value="quantity_desc" {{ $sort_oqrderQty== "quantity_desc"?'selected':''}}>{{\App\CPU\translate('quantity_sort_by_(بالكمية من الاعلي للقل)')}}</option>
                                <option value="order_asc" {{ $sort_oqrderQty== "order_asc"?'selected':''}}>{{\App\CPU\translate('order_sort_by_(عدد الطلبات من الاقل للاعلي)')}}</option>
                                <option value="order_desc" {{ $sort_oqrderQty== "order_desc"?'selected':''}}>{{\App\CPU\translate('order_sort_by_(عدد الطلبات من الاعلي للاقل)')}}</option>
                            </select>
                       </div>
