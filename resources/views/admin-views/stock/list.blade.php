                        ...
                        <div class="row justify-content-between align-items-center flex-grow-1">
                            ...
                            <div class="mt-1 col-12 col-sm-4">
                                <select name="qty_ordr_sort" class="form-control" onchange="location.href='{{ url('/') }}/admin/stock/stock-limit/?sort_oqrderQty='+this.value">
                                    <option value="default" {{ $sort_oqrderQty== "default"?'selected':''}}>{{\App\CPU\translate('افتراضي')}}</option>
                                    <option value="quantity_asc" {{ $sort_oqrderQty== "quantity_asc"?'selected':''}}>{{\App\CPU\translate('quantity_sort_by_(كمية من الاقل للاعلي)')}}</option>
                                <option value="quantity_desc" {{ $sort_oqrderQty== "quantity_desc"?'selected':''}}>{{\App\CPU\translate('quantity_sort_by_(كمية من الاعلي للاقل)')}}</option>
                                <option value="order_asc" {{ $sort_oqrderQty== "order_asc"?'selected':''}}>{{\App\CPU\translate('order_sort_by_(عدد مرات البيع من الاقل للاعلي)')}}</option>
                                <option value="order_desc" {{ $sort_oqrderQty== "order_desc"?'selected':''}}>{{\App\CPU\translate('order_sort_by_(عدد مرات البيع من الاعلي للاقل)')}}</option>
                                </select>
                            </div>
                        ...
                        <table class="table table-borderless table-thead-bordered table-nowrap table-align-middle card-table">
                            ...
                            @foreach($products as $key=>$product)
                                <tr>
                                    ...
                                    <td>
                                        {{ $product->supplier?$product->supplier->name:\App\CPU\translate('لايوجد') }} <br>
                                        {{ $product->supplier?$product->supplier->mobile:\App\CPU\translate('لايوجد')  }}
                                    </td>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">{{\App\CPU\translate('عدل كمية المنتج')}} <br>
                    <span class="text-danger">({{\App\CPU\translate('لكي تقلل كميةة المنتج استخدم._Ex: -10')}} )</span>
                </h5>
